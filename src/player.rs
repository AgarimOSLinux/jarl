            ...
            .and_then(|resp| {
                ...
                let reader: Box<dyn Read + Send + Sync + 'static> =
                    if let Some(mi) = metaint {
                        Box::new(IcyReader::new(resp.into_body().into_reader(), mi, Arc::clone(&track_title)))
                    } else {
                        Box::new(resp.into_body().into_reader())
                    };
                RadioStream::probe(reader, hint, Arc::clone(&track_title))
            })
            .or_else(|e| {
                    ...
                    || e.contains("invalid HTTP")
                {
                    ...
                    crate::icy::connect(&url)
                        .and_then(|reader| RadioStream::probe(reader, mime.as_deref(), Arc::clone(&track_title)))
                } else {
                    Err(e)
                }
            });
